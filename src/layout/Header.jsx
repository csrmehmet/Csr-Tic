  ...
  {desktopMenuData.map((item, index) => (
    item.subMenu ? (
      <div key={index} className="relative group">
        <Link to={item.link} className="text-light font-semibold hover:text-primary py-2">
          {item.label}
        </Link>
        {/* Padding değeri ve pointer-events-none ekledik */}
        <div className="absolute hidden group-hover:block bg-white shadow-lg pt-4 z-50 w-[600px] left-0">
          {/* Pointer-events-auto ekledik */}
          <div className="p-6 grid grid-cols-3 gap-8 pointer-events-auto">
            {item.subMenu.map((category, catIndex) => (
              <div key={catIndex} className="flex-1">
                {category.link ? (
                  <Link 
                    to={category.link}
                    className="block font-bold text-primary mb-4 hover:text-primary-dark"
                  >
                    {category.title}
                  </Link>
                ) : (
